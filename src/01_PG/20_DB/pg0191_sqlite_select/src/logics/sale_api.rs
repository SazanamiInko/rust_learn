...
use crate::logics::components::datamodel::to_datamodel::ToDataModel;
...
use super::components::verifys::param_verifys::file_exists_verify::FileExxistsVerify;
...
pub fn add_sale_record(take_on:i32,take_off:i32)->Result<(),Box<dyn Error>>
{
    ...
    let to_rang_verifye=RangeVerify::set("降駅",
                                              take_off,
                                               const_list::NORTH_STATION,
                                               const_list::SOUTH_STATION);
    //同値検査
...
//発駅検索
pub fn get_by_from(date:&str,take_on:i32)->Result<Vec<ToDataModel>,Box<dyn Error>>
{
    ...
    let file_exists_verify=FileExxistsVerify:: set(date);

    from_range_verify.verify()?;
    file_exists_verify.verify()?;

    // データを検索
    let path=util::get_sale_data_path(&date);
    let conn = Connection::open(&path)?;
    let mut statement = conn.prepare("SELECT id,take_off, time FROM sales WHERE take_on = ?1")?;
    let models = statement.query_map([take_on],|row| 
        {
            Ok(
                ToDataModel::new(row.get(0)?,
                             row.get(1)?,
    ...
    for datamodel_result in models
    {
        result_collection.push(datamodel_result?);
    }
           
return Ok(result_collection);
