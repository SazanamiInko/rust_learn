...
use crate::logics::components::remove_ferica_component::RemoveFericaComponent;
...
///フェリカの物理削除
pub fn remove_ferica()->Result<u32,Box<dyn Error>>
{
    //設定ファイルの読み込み
    let setting=setting::load();
    //トランザクションの作成
    ...
    //コンポーネントの生成
    let component=RemoveFericaComponent::new();
    
    let ret= component.execute(&mut tran);
